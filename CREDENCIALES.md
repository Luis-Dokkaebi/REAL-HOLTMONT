...
| `SAIRA` | `saira3725` | STAFF_USER | TRACKER | FACTURACION | (usuario legacy — usar ZAIRA_AGUILAR) |
...
> Movimiento de departamento: `CARLOS_MENDEZ` pasó de CONSTRUCCION a CALIDAD. `DIMAS_RAMOS` pasó de EHS a RH. Re-alta: `LILIANA_MARTINEZ` (RH), `LAURA_HUERTA` (RH), `ROCIO_CASTRO` (FINANZAS). Nombres legales actualizados: `DIMAS_RAMOS` → Dimas Eliel Ramos Garcia, `JUANY_RODRIGUEZ` → Juana Maria Rodriguez Juarez, `DANIA_GONZALEZ` → Dania Lizbeth Gonzalez Lores. Nuevo usuario `ZAIRA_AGUILAR` creado para Zaira Yazmin Aguilar Aguilon (FACTURACION).
