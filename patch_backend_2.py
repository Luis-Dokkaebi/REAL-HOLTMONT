...
# 2. Reverse Sync internalUpdateTask using basic replace instead of regex to avoid escape issues
old_internal_rev = """        } else if (String(personName).toUpperCase().includes("(VENTAS)")) {
...
code = code.replace(old_internal_rev, new_internal_rev)

# 3. Forward sync internalBatchUpdateTasks
old_batch_fwd = """             delete distData['PROCESO'];
             distributionTasks.push(distData);
        } else if (String(personName).toUpperCase().includes("(VENTAS)")) {"""

new_batch_fwd = """             delete distData['PROCESO'];
             if (taskData._assignToWorker && taskData._assignStep) {
                 try {
                     const assignData = JSON.parse(JSON.stringify(distData));
                     assignData['ESTATUS'] = 'PENDIENTE';
...
code = code.replace(old_batch_fwd, new_batch_fwd)

# 4. Reverse Sync internalBatchUpdateTasks
old_batch_rev = """          // Handle Reverse Sync (Vendor -> Antonia)
...
new_batch_rev = """          // Handle Reverse Sync (Vendor -> Antonia)
...
code = code.replace(old_batch_rev, new_batch_rev)
