...
function onOpen() {
  ...
  const menu = ui.createMenu('⚡ HOLTMONT CMD')
...
function importarPPCExterno() {
  ...
  try {
    ...
    const ui = SpreadsheetApp.getUi();
    ui.alert("No se pudo abrir la hoja externa. Verifica permisos.");
    return { success: false, message: "No se pudo abrir la hoja externa." };
  }
  ...
  if (payloads.length > 0) {
      try {
        const res = apiSavePPCData(payloads, 'JESUS_CANTU');
        if (res.success) {
            const ui = SpreadsheetApp.getUi();
            ui.alert(`✅ Importación completada. ${payloads.length} tareas procesadas.`);
            return { success: true, message: "Importación exitosa" };
        } else {
             console.error("Error en apiSavePPCData: " + res.message);
             return { success: false, message: res.message };
        }
      } catch (e) {
          console.error("Error guardando batch: " + e.message);
          return { success: false, message: e.message };
      }
  } else {
      const ui = SpreadsheetApp.getUi();
      ui.alert("⚠️ No se encontraron tareas nuevas para importar.");
      return { success: true, message: "Sin datos nuevos" };
  }
}
